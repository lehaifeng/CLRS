...
def load_task(data_path, label_path):
    task_num = os.listdir(label_path)  # 任务列表
...
def tw_variable(shape):
    with tf.name_scope('weights'):
        initial = tf.truncated_normal(shape, stddev=0.01)
    return tf.Variable(initial)


def tb_variable(shape):
...
class Model():
    def __init__(self, x, is_training, dropout):
        ...
        conv2_1 = residual_block(pool1, self.is_training, self.w2, self.w3, self.b2, self.b3, strides1=[1, 1, 1, 1],
                                 strides2=[1, 1, 1, 1], name1='conv2_1_1', name2='conv2_1_2')
        ...
        conv2_2 = residual_block(conv2_1, self.is_training, self.w4, self.w5, self.b4, self.b5, strides1=[1, 1, 1, 1],
                                 strides2=[1, 1, 1, 1], name1='conv2_2_1', name2='conv2_2_2')
        ...
        conv3_1 = residual_block_first(conv2_2, self.is_training, self.w7, self.w8, self.w6, self.b7, self.b8,
                                       strides1=[1, 2, 2, 1], strides2=[1, 1, 1, 1], name1='conv3_1_1',
                                       name2='conv3_1_2', name3='conv3_1_3')
        ...
        conv3_2 = residual_block(conv3_1, self.is_training, self.w9, self.w10, self.b9, self.b10, strides1=[1, 1, 1, 1],
                                 strides2=[1, 1, 1, 1], name1='conv3_2_1', name2='conv3_2_2')
        ...
        conv4_1 = residual_block_first(conv3_2, self.is_training, self.w12, self.w13, self.w11, self.b12, self.b13,
                                       strides1=[1, 2, 2, 1], strides2=[1, 1, 1, 1], name1='conv4_1_1',
                                       name2='conv4_1_2', name3='conv4_1_3')
        ...
        conv4_2 = residual_block(conv4_1, self.is_training, self.w14, self.w15, self.b14, self.b15,
                                 strides1=[1, 1, 1, 1], strides2=[1, 1, 1, 1], name1='conv4_2_1', name2='conv4_2_2')
        ...
        conv5_1 = residual_block_first(conv4_2, self.is_training, self.w17, self.w18, self.w16, self.b17, self.b18,
                                       strides1=[1, 2, 2, 1], strides2=[1, 1, 1, 1], name1='conv5_1_1',
                                       name2='conv5_1_2', name3='conv5_1_3')
        ...
        conv5_2 = residual_block(conv5_1, self.is_training, self.w19, self.w20, self.b19, self.b20,
                                 strides1=[1, 1, 1, 1], strides2=[1, 1, 1, 1], name1='conv5_2_1', name2='conv5_2_2')
...
def optimizer(lr_start, cost, varlist, steps, epoches):
    ...
    with tf.control_dependencies(update_ops):
        ...
        train_step = tf.train.GradientDescentOptimizer(learning_rate).minimize(cost, global_step=steps,
                                                                               var_list=varlist)
...
label_path = r'./CLRS dataset/label/NC/run1'  # NC/NIC
...
# expand output layer
W_output = tw_variable(shape=[1024, out_dim])
b_output = tb_variable([out_dim])
task_output = tf.matmul(model.drop1, W_output) + b_output

cw_w = tf.Variable(tf.zeros(shape=[1024, out_dim], dtype=tf.float32))

cw_b = tf.Variable(tf.zeros(shape=[out_dim], dtype=tf.float32))
...
for task in range(num_tasks_to_run):
    ...
    if task == 0:
        train_op, learning_rate = optimizer(lr_start=learning_lr, cost=cost, varlist=[model.params, W_output, b_output],
                                            steps=lr_steps, epoches=epoches)

    else:
        learning_lr=0.01
        sess.run(tf.variables_initializer([W_output, b_output]))
        train_op, learning_rate = optimizer(lr_start=learning_lr, cost=cost,
                                            varlist=[W_output, b_output], steps=lr_steps,
                                            epoches=epoches)
    ...
    # NC 场景
    for pp in set(train_task[task][1]):
        sess.run(cw_w[:, pp].assign(W_output[:, pp]))
        sess.run(cw_b[pp].assign(b_output[pp]))
    new_output = tf.matmul(model.drop1, cw_w) + cw_b

    # Print test set accuracy to each task encountered so far
    correct_prediction = tf.equal(tf.argmax(new_output, 1), tf.argmax(y_, 1))
    ...
    for k in range(0, iters):
        ...
        acc_ = sess.run(accuracy,
                        feed_dict={x: test_batch_images, y_: test_batch_labels, is_training: False, dropout: 1.0})
        te_acc.append(acc_)
    test_avg = np.mean(te_acc)
    print("Task: ", (task + 1), " \tAccuracy: ", test_avg)
    task_acc.append(test_avg)
    # save best model-test log
file = open('CLRS_CWR_reslution_NC.txt', 'w')
for fp in task_acc:
    file.write(str(fp) + ',')
file.close()
sess.close()