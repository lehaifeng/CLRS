...
# num_epochs_per_task =[300,400,500,500,500,500,500,500,500,500,500,500,600,600,600]
...
label_path=r'./CLRS dataset/label/NIC/run1'  #NC/NIC
...
# cw_w = 0.0
# cw_b = 0.0
class_count=np.zeros([25,])#NIC统计类出现的个数
...
for task in range(num_tasks_to_run):
    ...
    #NIC
    for nn in set(train_task[task][1]):
        class_count[nn] = class_count[nn] + 1
    ...
    for pp in set(train_task[task][1]):
        sess.run(cw_w[:, pp].assign((cw_w[:, pp]*class_count[pp]+W_output[:, pp])/ (class_count[pp] + 1)))
        sess.run(cw_b[pp].assign((cw_b[pp]*class_count[pp]+b_output[pp])/ (class_count[pp] + 1)))
...
file = open('CLRS_CWR_reslution_NIC.txt', 'w')
for fp in task_acc:
    file.write(str(fp)+',')
file.close()
sess.close()