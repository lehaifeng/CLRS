...
def compute_omega(sess, imgset, batch_size, param_im):
    ...
    for iter in range(batch_num):
        ...
        Omega_M.append(
            sess.run(param_im, feed_dict={x: imgset[index:(index + batch_size)], is_training: True, dropout: 0.5}))
...
# NIC:15
# num_tasks_to_run = 15 #任务数
num_epochs_per_task = [300,320,320,330,340,350,360,370,380,390,400,400,420,450,450]
minibatch_size = 64
epoches = 2000
learning_lr = 0.001
Lamb_set = [5]
...
label_path = r'./CLRS dataset/label/NIC/run2'  # NI/NC/NIC
...
for Lamb in Lamb_set:
    ...
    file = open('CLRS_ewc_reslution_NIC.txt', 'w')
    for fp in task_acc:
        file.write(str(fp) + ',')
    file.close()
    sess.close()